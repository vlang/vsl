...
pub fn gemm_cuda(dev &cuda.Device, a_data []f64, b_data []f64, m int, n int, k int) ![]f64 {
	...
	mut d := dev
	c_col := cuda_compute.gemm_cuda_impl(mut d, a_col, b_col, m, n, k)!
...
pub fn gemv_cuda(dev &cuda.Device, a_data []f64, x_data []f64, m int, n int) ![]f64 {
	a_col := row_to_col_major(a_data, m, n)
	mut d := dev
	return cuda_compute.gemv_cuda_impl(mut d, a_col, x_data, m, n)
}