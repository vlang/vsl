...
fn main() {
	// Arbitrary values. Since ml.KNN only takes floats, we must represent
	...
	// using ml.remove_punctuation and set everything to lowercase. However,
	...
	mut labels := []f64{}
	for row in dataset {
		tokenized := ml.tokenize(ml.remove_punctuation(row[0]).to_lower())
		// To specify custom stopwords for your language, run:
		// ml.remove_stopwords(tokens []string, stopwords []string, ignore_case bool).
		tokens << ml.remove_stopwords_en(
			tokenized,
			true // Last parameter is a flag to set everything to lowercase.
				 // If set to false, case is kept.
		)
		labels << class[row[1]]
	}
	...
	mut lancaster := ml.new_lancaster_stemmer(true) // Parameter is strip_prefix.
													// If true, "kilogram" becomes
													// "gram", for example.

	mut ngrams := [][][]string{}		// List of sentences as ngrams, read the comments
										// below to understand.
	...
	for i in 0..dataset.len {
		...
		// The option/result below is in case you pass negative values to ml.ngrams.
		// TODO: ask Ulises to convert it back to a vsl panic.
		ng := ml.ngrams(stemmed, 1) or { panic(err) } // [][]string
	...
	// bag of words. It is implemented using ml.most_frequent_ngrams and ml.count_vectorize.
	// We need to specify a number of features (that is, how many ngrams will be
	// returned by ml.most_frequent_ngrams). Since in this case the decisive tokens
	...
	n_features := 5
	mut most_freq := ml.most_frequent_ngrams(all_ngrams, n_features) or { panic(err) } // [][]string
	...
	for i in 0..dataset.len {
		...
		// return type of ml.count_vectorize) to f64, to feed to our KNN model
		// in the next steps.
		vectorized << ml.count_vectorize(ngrams[i], most_freq).map(f64(it))
	}
	...
	// even exist?)
	bow := fn(sent string, mut lan ml.LancasterStemmer, mf [][]string) []f64 {
		sent_tokenized := ml.remove_stopwords_en(
			ml.tokenize(ml.remove_punctuation(sent).to_lower()),
			true
		)
		...
		sent_ngrams := ml.ngrams(sent_stemmed, 1) or { panic(err) }
		return ml.count_vectorize(sent_ngrams, mf).map(f64(it))
	}

	bow_prediction1 := bow_knn.predict(
		k: 2, // low value due to small dataset
	...
	for sent in ngrams {
		...
		for u_ng in unique_ngrams {
			tf_idf_sentence << ml.tf_idf(u_ng, sent, ngrams) or { panic(err) }
		}
		tf_idf_rows << tf_idf_sentence
	}

	training_data = ml.data_from_raw_xy_sep(tf_idf_rows, labels)
	mut tf_idf_knn := ml.new_knn(
		mut training_data,
		'TfIdfKNN'
	)

	tfidf := fn(sent string, mut lan ml.LancasterStemmer, document [][][]string, unique [][]string) []f64 {
		sent_tokenized := ml.remove_stopwords_en(
			ml.tokenize(ml.remove_punctuation(sent).to_lower()),
			true
		)
		...
		mut sent_ng := ml.ngrams(sent_stemmed, 1) or { panic(err) }
		mut tf_idf_sentence := []f64{}
		for u_ng in unique {
			tf_idf_sentence << ml.tf_idf(u_ng, sent_ng, document) or { panic(err) }
		}
