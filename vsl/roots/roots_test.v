...
fn test_root_bisection() {
        x1 := f64(0)
        x2 := f64(3)
        func := vsl.Function{function: f_cos}

        result := roots.bisection(func, x1, x2, epsrel, epsabs, n_max) or { panic(err) }
...
fn test_root_newton() {
        x0 := f64(0.5)
        func := vsl.FunctionFdf{fdf: fdf_cos}

        result := roots.newton(func, x0, epsrel, epsabs, n_max) or { panic(err) }
