...
static void my_sincosm1pi_f32 (float a, float *restrict res)
  {
  float s = a * a;
  /* Approximate cos(pi*x)-1 for x in [-0.25,0.25] */
/* copy of the double which is not used ...
  float r =     -1.0369917389758117e-4;
  r = fmaf (r, s,  1.9294935641298806e-3);
  r = fmaf (r, s, -2.5806887942825395e-2);
  r = fmaf (r, s,  2.3533063028328211e-1);
  r = fmaf (r, s, -1.3352627688538006e+0);
  r = fmaf (r, s,  4.0587121264167623e+0);
  r = fmaf (r, s, -4.9348022005446790e+0);
*/
...
/* copy of the double which is not used...
  ...
  r = fmaf (r, s, -7.3700183130883555e-3);
  r = fmaf (r, s,  8.2145868949323936e-2);
  r = fmaf (r, s, -5.9926452893214921e-1);
  r = fmaf (r, s,  2.5501640398732688e+0);
  r = fmaf (r, s, -5.1677127800499516e+0);
*/
  r =             -0x1.310000p-1f;
...
NOINLINE static void calc_first_octant_f32(size_t den, float * restrict res)
  {
  size_t n = (den+4)>>3;
  if (n==0) return;
  res[0]=1.; res[1]=0.;
  ...
  for (size_t i=1; i<l1; ++i)
    my_sincosm1pi_f32((2.*i)/den,&res[2*i]);
  size_t start=l1;
  while(start<n)
    {
    float cs[2];
    my_sincosm1pi_f32((2.*start)/den,cs);
    res[2*start] = cs[0]+1.;
    res[2*start+1] = cs[1];
    size_t end = l1;
...
NOINLINE static void calc_first_quadrant_f32(size_t n, float * restrict res)
  {
  float * restrict p = res+n;
  calc_first_octant_f32(n<<1, p);
  size_t ndone=(n+2)>>2;
...
NOINLINE static void calc_first_half_f32(size_t n, float * restrict res)
  {
  int ndone=(n+1)>>1;
  float * p = res+n-1;
  calc_first_octant_f32(n<<2, p);
  int i4=0, in=n, i=0;
...
NOINLINE static void fill_first_quadrant_f32(size_t n, float * restrict res)
  {
...
NOINLINE static void fill_first_half_f32(size_t n, float * restrict res)
  {
  size_t half = n>>1;
  if ((n&3)==0)
...
NOINLINE static void fill_second_half_f32(size_t n, float * restrict res)
  {
  if ((n&1)==0)
...
NOINLINE static void sincos_2pibyn_f32_half_f32(size_t n, float * restrict res)
  {
  if ((n&3)==0)
    {
    calc_first_octant_f32(n, res);
    fill_first_quadrant_f32(n, res);
    fill_first_half_f32(n, res);
    }
  else if ((n&1)==0)
    {
    calc_first_quadrant_f32(n, res);
    fill_first_half_f32(n, res);
    }
  else
    calc_first_half_f32(n, res);
  }

NOINLINE static void sincos_2pibyn_f32(size_t n, float * restrict res)
  {
  sincos_2pibyn_f32_half_f32(n, res);
  fill_second_half_f32(n, res);
  }
...
NOINLINE static void pass2b_f32 (size_t ido, size_t l1, const cmplx_f32 * restrict cc,
  cmplx_f32 * restrict ch, const cmplx_f32 * restrict wa)
  {
  const size_t cdim=2;
...
NOINLINE static void pass2f_f32 (size_t ido, size_t l1, const cmplx_f32 * restrict cc,
  cmplx_f32 * restrict ch, const cmplx_f32 * restrict wa)
  {
  const size_t cdim=2;
...
NOINLINE static void pass3b_f32 (size_t ido, size_t l1, const cmplx_f32 * restrict cc,
  cmplx_f32 * restrict ch, const cmplx_f32 * restrict wa)
  {
  const size_t cdim=3;
...
NOINLINE static void pass3f_f32 (size_t ido, size_t l1, const cmplx_f32 * restrict cc,
  cmplx_f32 * restrict ch, const cmplx_f32 * restrict wa)
  {
  const size_t cdim=3;
  const float tw1r=-0.5, tw1i= -0.86602540378443864676;
...
NOINLINE static void pass4b_f32 (size_t ido, size_t l1, const cmplx_f32 * restrict cc,
  cmplx_f32 * restrict ch, const cmplx_f32 * restrict wa)
  {
  const size_t cdim=4;
  ...
  if (ido==1)
    ...
    for (size_t k=0; k<l1; ++k)
      ...
      for (size_t i=1; i<ido; ++i)
        {
        cmplx_f32 c2, c3, c4, t1, t2, t3, t4;
        cmplx_f32 cc0=CC(i,0,k), cc1=CC(i,1,k),cc2=CC(i,2,k),cc3=CC(i,3,k);
        PMC(t2,t1,cc0,cc2)
        PMC(t3,t4,cc1,cc3)
        ROT90(t4)
        cmplx_f32 wa0=WA(0,i), wa1=WA(1,i),wa2=WA(2,i);
...
NOINLINE static void pass4f_f32 (size_t ido, size_t l1, const cmplx_f32 * restrict cc,
  cmplx_f32 * restrict ch, const cmplx_f32 * restrict wa)
  {
  const size_t cdim=4;
...
#define PREP5(idx) \
        cmplx_f32 t0 = CC(idx,0,k), t1, t2, t3, t4; \
...
NOINLINE static void pass5b_f32 (size_t ido, size_t l1, const cmplx_f32 * restrict cc,
  cmplx_f32 * restrict ch, const cmplx_f32 * restrict wa)
  {
  const size_t cdim=5;
...
NOINLINE static void pass5f_f32 (size_t ido, size_t l1, const cmplx_f32 * restrict cc,
  cmplx_f32 * restrict ch, const cmplx_f32 * restrict wa)
  {
  const size_t cdim=5;
  const float tw1r= 0.3090169943749474241,
...
#define PREP7(idx) \
        cmplx_f32 t1 = CC(idx,0,k), t2, t3, t4, t5, t6, t7; \
...
NOINLINE static void pass7(size_t ido, size_t l1, const cmplx_f32 * restrict cc,
  cmplx_f32 * restrict ch, const cmplx_f32 * restrict wa, const int sign)
  {
  const size_t cdim=7;
...
#define PREP11(idx) \
        cmplx_f32 t1 = CC(idx,0,k), t2, t3, t4, t5, t6, t7, t8, t9, t10, t11; \
...
NOINLINE static void pass11_f32 (size_t ido, size_t l1, const cmplx_f32 * restrict cc,
  cmplx_f32 * restrict ch, const cmplx_f32 * restrict wa, const int sign)
  {
...
NOINLINE static int passg_f32 (size_t ido, size_t ip, size_t l1,
  cmplx_f32 * restrict cc, cmplx_f32 * restrict ch, const cmplx_f32 * restrict wa,
  const cmplx_f32 * restrict csarr, const int sign)
  {
  const size_t cdim=ip;
  size_t ipph = (ip+1)/2;
  size_t idl1 = ido*l1;

  cmplx_f32 * restrict wal=RALLOC(cmplx_f32,ip);
  if (!wal) return -1;
  wal[0]=(cmplx_f32){1.,0.};
  for (size_t i=1; i<ip; ++i)
    wal[i]=(cmplx_f32){csarr[i].r,sign*csarr[i].i};
  ...
  for (size_t l=1, lc=ip-1; l<ipph; ++l, --lc)
    ...
    for (; j<ipph-1; j+=2, jc-=2)
      ...
      cmplx_f32 xwal=wal[iwal];
      iwal+=l; if (iwal>ip) iwal-=ip;
      cmplx_f32 xwal2=wal[iwal];
      for (size_t ik=0; ik<idl1; ++ik)
        {
  ...
  if (ido==1)
    for (size_t j=1, jc=ip-1; j<ipph; ++j, --jc)
      for (size_t ik=0; ik<idl1; ++ik)
        {
        cmplx_f32 t1=CX2(ik,j), t2=CX2(ik,jc);
    ...
    for (size_t j=1, jc=ip-1; j<ipph; ++j,--jc)
      for (size_t k=0; k<l1; ++k)
        {
        cmplx_f32 t1=CX(0,k,j), t2=CX(0,k,jc);
...
NOINLINE WARN_UNUSED_RESULT static int pass_all_f32(cfftp_plan_f32 plan, cmplx_f32 c[], float fct,
  const int sign)
  {
  if (plan->length==1) return 0;
  size_t len=plan->length;
  size_t l1=1, nf=plan->nfct;
  cmplx_f32 *ch = RALLOC(cmplx_f32, len);
  if (!ch) return -1;
  cmplx_f32 *p1=c, *p2=ch;
  ...
  for(size_t k1=0; k1<nf; k1++)
    ...
    size_t ido = len/l2;
    if     (ip==4)
      sign>0 ? pass4b_f32 (ido, l1, p1, p2, plan->fct[k1].tw)
             : pass4f_f32 (ido, l1, p1, p2, plan->fct[k1].tw);
    else if(ip==2)
      sign>0 ? pass2b_f32 (ido, l1, p1, p2, plan->fct[k1].tw)
             : pass2f_f32 (ido, l1, p1, p2, plan->fct[k1].tw);
    else if(ip==3)
      sign>0 ? pass3b_f32 (ido, l1, p1, p2, plan->fct[k1].tw)
             : pass3f_f32 (ido, l1, p1, p2, plan->fct[k1].tw);
    else if(ip==5)
      sign>0 ? pass5b_f32 (ido, l1, p1, p2, plan->fct[k1].tw)
             : pass5f_f32 (ido, l1, p1, p2, plan->fct[k1].tw);
    else if(ip==7)  pass7 (ido, l1, p1, p2, plan->fct[k1].tw, sign);
    else if(ip==11) pass11_f32(ido, l1, p1, p2, plan->fct[k1].tw, sign);
    else
      {
      if (passg_f32(ido, ip, l1, p1, p2, plan->fct[k1].tw, plan->fct[k1].tws, sign))
        { DEALLOC(ch); return -1; }
      SWAP(p1,p2,cmplx_f32 *);
      }
    SWAP(p1,p2,cmplx_f32 *);
    l1=l2;
    }
  if (p1!=c)
    ...
    if (fct!=1.)
      ...
      memcpy (c,p1,len*sizeof(cmplx_f32));
    }
  else
    if (fct!=1.)
      for (size_t i=0; i<len; ++i)
...
static int cfftp_forward_f32(cfftp_plan_f32 plan, float c[], float fct)
  { return pass_all_f32(plan,(cmplx_f32 *)c, fct, -1); }

NOINLINE WARN_UNUSED_RESULT
static int cfftp_backward_f32(cfftp_plan_f32 plan, float c[], float fct)
  { return pass_all_f32(plan,(cmplx_f32 *)c, fct, 1); }

NOINLINE WARN_UNUSED_RESULT
static int cfftp_factorize_f32 (cfftp_plan_f32 plan)
  {
...
NOINLINE static size_t cfftp_twsize_f32 (cfftp_plan_f32 plan)
  {
  size_t twsize_f32=0, l1=1;
  for (size_t k=0; k<plan->nfct; ++k)
    {
    size_t ip=plan->fct[k].fct, ido= plan->length/(l1*ip);
    twsize_f32+=(ip-1)*(ido-1);
    if (ip>11)
      twsize_f32+=ip;
    l1*=ip;
    }
...
NOINLINE WARN_UNUSED_RESULT static int cfftp_comp_twiddle_f32 (cfftp_plan_f32 plan)
  {
  size_t length=plan->length;
  float *twid = RALLOC(float, 2*length);
  if (!twid) return -1;
  sincos_2pibyn_f32(length, twid);
  size_t l1=1;
  size_t memofs=0;
  for (size_t k=0; k<plan->nfct; ++k)
    {
...
static cfftp_plan_f32 make_cfftp_plan_f32 (size_t length)
  {
  if (length==0) return NULL;
  cfftp_plan_f32 plan = RALLOC(cfftp_plan_i_f32,1);
  if (!plan) return NULL;
  plan->length=length;
  plan->nfct=0;
  ...
  if (cfftp_factorize_f32(plan)!=0) { DEALLOC(plan); return NULL; }
  size_t tws=cfftp_twsize_f32(plan);
  plan->mem=RALLOC(cmplx_f32,tws);
  if (!plan->mem) { DEALLOC(plan); return NULL; }
  if (cfftp_comp_twiddle_f32(plan)!=0)
    { DEALLOC(plan->mem); DEALLOC(plan); return NULL; }
  return plan;
  }

static void destroy_cfftp_plan_f32 (cfftp_plan_f32 plan)
  {
  DEALLOC(plan->mem);
  DEALLOC(plan);
  }
...
NOINLINE static void radf2_f32 (size_t ido, size_t l1, const float * restrict cc,
...
NOINLINE static void radf3_f32(size_t ido, size_t l1, const float * restrict cc,
  float * restrict ch, const float * restrict wa)
  {
  const size_t cdim=3;
...
NOINLINE static void radf4_f32(size_t ido, size_t l1, const float * restrict cc,
  float * restrict ch, const float * restrict wa)
  {
  const size_t cdim=4;
...
NOINLINE static void radf5_f32(size_t ido, size_t l1, const float * restrict cc,
  float * restrict ch, const float * restrict wa)
  {
  const size_t cdim=5;
...
#define CC(a,b,c) cc[(a)+ido*((b)+cdim*(c))]
#define CH(a,b,c) ch[(a)+ido*((b)+l1*(c))]
NOINLINE static void radfg_f32(size_t ido, size_t ip, size_t l1,
...
NOINLINE static void radb2_f32(size_t ido, size_t l1, const float * restrict cc,
  float * restrict ch, const float * restrict wa)
  {
  const size_t cdim=2;
...
NOINLINE static void radb3_f32(size_t ido, size_t l1, const float * restrict cc,
  float * restrict ch, const float * restrict wa)
  {
  const size_t cdim=3;
  static const float taur=-0.5, taui=0.86602540378443864676;
...
NOINLINE static void radb4_f32(size_t ido, size_t l1, const float * restrict cc,
  float * restrict ch, const float * restrict wa)
  {
  const size_t cdim=4;
...
NOINLINE static void radb5_f32(size_t ido, size_t l1, const float * restrict cc,
  float * restrict ch, const float * restrict wa)
  {
  const size_t cdim=5;
  static const float tr11= 0.3090169943749474241, ti11=0.95105651629515357212,
...
NOINLINE static void radbg_f32(size_t ido, size_t ip, size_t l1,
  float * restrict cc, float * restrict ch, const float * restrict wa,
  const float * restrict csarr)
  {
  const size_t cdim=ip;
...
static void copy_and_norm_f32_f32(float *c, float *p1, size_t n, float fct)
  {
  if (p1!=c)
    {
    if (fct!=1.)
...
WARN_UNUSED_RESULT
static int rfftp_forward_f32(rfftp_plan_f32 plan, float c[], float fct)
  {
  if (plan->length==1) return 0;
  ...
  for(size_t k1=0; k1<nf;++k1)
    ...
    l1 /= ip;
    if(ip==4)
      radf4_f32(ido, l1, p1, p2, plan->fct[k].tw);
    else if(ip==2)
      radf2_f32(ido, l1, p1, p2, plan->fct[k].tw);
    else if(ip==3)
      radf3_f32(ido, l1, p1, p2, plan->fct[k].tw);
    else if(ip==5)
      radf5_f32(ido, l1, p1, p2, plan->fct[k].tw);
    else
      {
      radfg_f32(ido, ip, l1, p1, p2, plan->fct[k].tw, plan->fct[k].tws);
      SWAP (p1,p2,float *);
      }
    SWAP (p1,p2,float *);
    }
  copy_and_norm_f32_f32(c,p1,n,fct);
  DEALLOC(ch);
  return 0;
  }

WARN_UNUSED_RESULT
static int rfftp_backward_f32(rfftp_plan_f32 plan, float c[], float fct)
  {
  if (plan->length==1) return 0;
  size_t n=plan->length;
  size_t l1=1, nf=plan->nfct;
  ...
  for(size_t k=0; k<nf; k++)
    ...
    size_t ip = plan->fct[k].fct,
           ido= n/(ip*l1);
    if(ip==4)
      radb4_f32(ido, l1, p1, p2, plan->fct[k].tw);
    else if(ip==2)
      radb2_f32(ido, l1, p1, p2, plan->fct[k].tw);
    else if(ip==3)
      radb3_f32(ido, l1, p1, p2, plan->fct[k].tw);
    else if(ip==5)
      radb5_f32(ido, l1, p1, p2, plan->fct[k].tw);
    else
      radbg_f32(ido, ip, l1, p1, p2, plan->fct[k].tw, plan->fct[k].tws);
    SWAP (p1,p2,float *);
    l1*=ip;
    }
  copy_and_norm_f32_f32(c,p1,n,fct);
...
WARN_UNUSED_RESULT
static int rfftp_factorize_f32 (rfftp_plan_f32 plan)
  {
  size_t length=plan->length;
  size_t nfct=0;
  while ((length%4)==0)
...
static size_t rfftp_twsize_f32(rfftp_plan_f32 plan)
  {
  size_t twsize_f32=0, l1=1;
  for (size_t k=0; k<plan->nfct; ++k)
    {
...
WARN_UNUSED_RESULT NOINLINE static int rfftp_comp_twiddle_f32 (rfftp_plan_f32 plan)
  {
  size_t length=plan->length;
  float *twid = RALLOC(float, 2*length);
  if (!twid) return -1;
  sincos_2pibyn_f32_half_f32(length, twid);
  size_t l1=1;
  float *ptr=plan->mem;
  for (size_t k=0; k<plan->nfct; ++k)
    {
    size_t ip=plan->fct[k].fct, ido=length/(l1*ip);
    if (k<plan->nfct-1) // last factor doesn't need twiddle_f32s
      {
...
NOINLINE static rfftp_plan_f32 make_rfftp_plan_f32 (size_t length)
  {
  if (length==0) return NULL;
  rfftp_plan_f32 plan = RALLOC(rfftp_plan_i_f32,1);
  if (!plan) return NULL;
  plan->length=length;
  plan->nfct=0;
  plan->mem=NULL;
  ...
  if (length==1) return plan;
  if (rfftp_factorize_f32(plan)!=0) { DEALLOC(plan); return NULL; }
  size_t tws=rfftp_twsize_f32(plan);
  plan->mem=RALLOC(float,tws);
  if (!plan->mem) { DEALLOC(plan); return NULL; }
  if (rfftp_comp_twiddle_f32(plan)!=0)
    { DEALLOC(plan->mem); DEALLOC(plan); return NULL; }
  return plan;
  }

NOINLINE static void destroy_rfftp_plan_f32 (rfftp_plan_f32 plan)
  {
  DEALLOC(plan->mem);
  DEALLOC(plan);
  }
...
NOINLINE static fftblue_plan_f32 make_fftblue_plan_f32 (size_t length)
  {
  fftblue_plan_f32 plan = RALLOC(fftblue_plan_i_f32,1);
  if (!plan) return NULL;
  plan->n = length;
...
/* initialize b_k */
  ...
  sincos_2pibyn_f32(2*plan->n,tmp);
  plan->bk[0] = 1;
  plan->bk[1] = 0;
  ...
  for (size_t m=2*plan->n;m<=(2*plan->n2-2*plan->n+1);++m)
    plan->bkf[m]=0.;
  plan->plan=make_cfftp_plan_f32(plan->n2);
  if (!plan->plan)
    { DEALLOC(tmp); DEALLOC(plan->mem); DEALLOC(plan); return NULL; }
  if (cfftp_forward_f32(plan->plan,plan->bkf,1.)!=0)
    { DEALLOC(tmp); DEALLOC(plan->mem); DEALLOC(plan); return NULL; }
  DEALLOC(tmp);
...
NOINLINE static void destroy_fftblue_plan_f32 (fftblue_plan_f32 plan)
  {
  DEALLOC(plan->mem);
  destroy_cfftp_plan_f32(plan->plan);
  DEALLOC(plan);
  }

NOINLINE WARN_UNUSED_RESULT
static int fftblue_fft_f32(fftblue_plan_f32 plan, float c[], int isign, float fct)
  {
  size_t n=plan->n;
  size_t n2=plan->n2;
  float *bk  = plan->bk;
...
/* initialize a_k and FFT it */
  ...
  if (cfftp_forward_f32 (plan->plan,akf,fct)!=0)
...
/* inverse FFT */
  if (cfftp_backward_f32 (plan->plan,akf,1.)!=0)
    { DEALLOC(akf); return -1; }
...
WARN_UNUSED_RESULT
static int cfftblue_backward_f32(fftblue_plan_f32 plan, float c[], float fct)
  { return fftblue_fft_f32(plan,c,1,fct); }

WARN_UNUSED_RESULT
static int cfftblue_forward_f32(fftblue_plan_f32 plan, float c[], float fct)
  { return fftblue_fft_f32(plan,c,-1,fct); }

WARN_UNUSED_RESULT
static int rfftblue_backward_f32(fftblue_plan_f32 plan, float c[], float fct)
  {
  size_t n=plan->n;
  ...
  if (fftblue_fft_f32(plan,tmp,1,fct)!=0)
...
WARN_UNUSED_RESULT
static int rfftblue_forward_f32(fftblue_plan_f32 plan, float c[], float fct)
  {
  size_t n=plan->n;
  float *tmp = RALLOC(float,2*n);
  if (!tmp) return -1;
...
cfft_plan_f32 make_cfft_plan_f32 (size_t length)
  {
  if (length==0) return NULL;
  cfft_plan_f32 plan = RALLOC(cfft_plan_i_f32,1);
  if (!plan) return NULL;
  plan->blueplan=0;
  plan->packplan=0;
  if ((length<50) || (largest_prime_factor(length)<=sqrt(length)))
    {
    plan->packplan=make_cfftp_plan_f32(length);
  ...
  if (comp2<comp1) // use Bluestein
    {
    plan->blueplan=make_fftblue_plan_f32(length);
...
void destroy_cfft_plan_f32 (cfft_plan_f32 plan)
  {
  if (plan->blueplan)
    destroy_fftblue_plan_f32(plan->blueplan);
  if (plan->packplan)
    destroy_cfftp_plan_f32(plan->packplan);
  DEALLOC(plan);
  }

WARN_UNUSED_RESULT int cfft_backward_f32(cfft_plan_f32 plan, float c[], float fct)
  {
  if (plan->packplan)
    return cfftp_backward_f32(plan->packplan,c,fct);
  // if (plan->blueplan)
  return cfftblue_backward_f32(plan->blueplan,c,fct);
  }

WARN_UNUSED_RESULT int cfft_forward_f32(cfft_plan_f32 plan, float c[], float fct)
  {
  if (plan->packplan)
    return cfftp_forward_f32(plan->packplan,c,fct);
  // if (plan->blueplan)
  return cfftblue_forward_f32(plan->blueplan,c,fct);
  }
...
rfft_plan_f32 make_rfft_plan_f32 (size_t length)
  {
  if (length==0) return NULL;
  rfft_plan_f32 plan = RALLOC(rfft_plan_i_f32,1);
  if (!plan) return NULL;
  plan->blueplan=0;
  plan->packplan=0;
  if ((length<50) || (largest_prime_factor(length)<=sqrt(length)))
    {
    plan->packplan=make_rfftp_plan_f32(length);
    if (!plan->packplan) { DEALLOC(plan); return NULL; }
    return plan;
    }
...
void destroy_rfft_plan_f32 (rfft_plan_f32 plan)
  ...
  if (plan->blueplan)
    destroy_fftblue_plan_f32(plan->blueplan);
  if (plan->packplan)
    destroy_rfftp_plan_f32(plan->packplan);
  DEALLOC(plan);
  }
...
WARN_UNUSED_RESULT int rfft_backward_f32(rfft_plan_f32 plan, float c[], float fct)
  {
  if (plan->packplan)
    return rfftp_backward_f32(plan->packplan,c,fct);
  else // if (plan->blueplan)
    return rfftblue_backward_f32(plan->blueplan,c,fct);
  }

WARN_UNUSED_RESULT int rfft_forward_f32(rfft_plan_f32 plan, float c[], float fct)
  {
  if (plan->packplan)
    return rfftp_forward_f32(plan->packplan,c,fct);
  else // if (plan->blueplan)
    return rfftblue_forward_f32(plan->blueplan,c,fct);
  }
